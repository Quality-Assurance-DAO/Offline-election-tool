//! Performance test: Sparse voting patterns (nominators vote for few candidates)
...
fn test_sparse_voting_patterns() {
    const CANDIDATE_COUNT: usize = 1_000;
    const NOMINATOR_COUNT: usize = 10_000;
    const VOTES_PER_NOMINATOR: usize = 3; // Each nominator votes for only 3 candidates (sparse)
    
    println!("Generating sparse voting data: {} candidates, {} nominators, {} votes per nominator", 
    ...
    // Generate nominators with sparse voting (each votes for few candidates)
    for i in 0..NOMINATOR_COUNT {
        ...
        // Sparse voting: vote for only VOTES_PER_NOMINATOR candidates
        // Distribute votes across candidates
        let targets: Vec<String> = (0..VOTES_PER_NOMINATOR)
            .map(|j| {
                let candidate_idx = (i * VOTES_PER_NOMINATOR + j) % CANDIDATE_COUNT;
                format!("5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY{}", candidate_idx)
            })
            .collect();
    ...
    metadata.insert("benchmark_name".to_string(), "sparse_voting".to_string());
    ...
    println!("✓ Sparse voting test passed: {}ms", execution_time_ms);
}
