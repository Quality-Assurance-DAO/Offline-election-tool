//! Performance test: 10k candidates and 100k nominators
//! Target: No OOM (Out of Memory)
...
fn test_large_scale_10k_candidates_100k_nominators() {
    const CANDIDATE_COUNT: usize = 10_000;
    const NOMINATOR_COUNT: usize = 100_000;
    ...
    // Main goal: should not OOM
    assert!(result.is_ok(), "Election should succeed without OOM");
    ...
    metadata.insert("benchmark_name".to_string(), "large_scale_10k".to_string());
    ...
    println!("✓ Performance test passed: No OOM, execution time: {}ms", execution_time_ms);
}
