//! Chain snapshot tests for Kusama

use super::*;
use offline_election::engine::ElectionEngine;
use offline_election::models::election_config::ElectionConfiguration;
use offline_election::types::AlgorithmType;
use crate::common::fixture_loader::load_chain_snapshot;
use crate::common::assertions::compare_results_exact_match;
use std::path::PathBuf;

const KUSAMA_RPC: &str = "https://kusama-rpc.polkadot.io";

#[test]
#[ignore] // Requires network access
async fn test_kusama_block_1() {
    let fixture_path = PathBuf::from("tests/fixtures/chain_snapshots/kusama/block_1.json");
    
    if !fixture_path.exists() {
        eprintln!("⚠ Fixture not found: {:?}. Skipping test.", fixture_path);
        return;
    }
    
    let snapshot = load_chain_snapshot(&fixture_path)
        .expect("Failed to load chain snapshot");
    
    let engine = ElectionEngine::new();
    let config = ElectionConfiguration {
        active_set_size: snapshot.expected_result.selected_validators.len() as u32,
        algorithm: AlgorithmType::SequentialPhragmen,
        overrides: None,
        block_number: Some(snapshot.metadata.block_number),
    };
    
    let result = engine.execute(&config, &snapshot.election_data)
        .expect("Election execution should succeed");
    
    compare_results_exact_match(&result, &snapshot.expected_result)
        .expect("Results should match exactly");
}

#[test]
#[ignore] // Requires network access
async fn test_kusama_block_2() {
    let fixture_path = PathBuf::from("tests/fixtures/chain_snapshots/kusama/block_2.json");
    
    if !fixture_path.exists() {
        eprintln!("⚠ Fixture not found: {:?}. Skipping test.", fixture_path);
        return;
    }
    
    let snapshot = load_chain_snapshot(&fixture_path)
        .expect("Failed to load chain snapshot");
    
    let engine = ElectionEngine::new();
    let config = ElectionConfiguration {
        active_set_size: snapshot.expected_result.selected_validators.len() as u32,
        algorithm: AlgorithmType::SequentialPhragmen,
        overrides: None,
        block_number: Some(snapshot.metadata.block_number),
    };
    
    let result = engine.execute(&config, &snapshot.election_data)
        .expect("Election execution should succeed");
    
    compare_results_exact_match(&result, &snapshot.expected_result)
        .expect("Results should match exactly");
}

#[test]
#[ignore] // Requires network access
async fn test_kusama_block_3() {
    let fixture_path = PathBuf::from("tests/fixtures/chain_snapshots/kusama/block_3.json");
    
    if !fixture_path.exists() {
        eprintln!("⚠ Fixture not found: {:?}. Skipping test.", fixture_path);
        return;
    }
    
    let snapshot = load_chain_snapshot(&fixture_path)
        .expect("Failed to load chain snapshot");
    
    let engine = ElectionEngine::new();
    let config = ElectionConfiguration {
        active_set_size: snapshot.expected_result.selected_validators.len() as u32,
        algorithm: AlgorithmType::SequentialPhragmen,
        overrides: None,
        block_number: Some(snapshot.metadata.block_number),
    };
    
    let result = engine.execute(&config, &snapshot.election_data)
        .expect("Election execution should succeed");
    
    compare_results_exact_match(&result, &snapshot.expected_result)
        .expect("Results should match exactly");
}

#[test]
#[ignore] // Requires network access
async fn test_kusama_block_4() {
    let fixture_path = PathBuf::from("tests/fixtures/chain_snapshots/kusama/block_4.json");
    
    if !fixture_path.exists() {
        eprintln!("⚠ Fixture not found: {:?}. Skipping test.", fixture_path);
        return;
    }
    
    let snapshot = load_chain_snapshot(&fixture_path)
        .expect("Failed to load chain snapshot");
    
    let engine = ElectionEngine::new();
    let config = ElectionConfiguration {
        active_set_size: snapshot.expected_result.selected_validators.len() as u32,
        algorithm: AlgorithmType::SequentialPhragmen,
        overrides: None,
        block_number: Some(snapshot.metadata.block_number),
    };
    
    let result = engine.execute(&config, &snapshot.election_data)
        .expect("Election execution should succeed");
    
    compare_results_exact_match(&result, &snapshot.expected_result)
        .expect("Results should match exactly");
}

